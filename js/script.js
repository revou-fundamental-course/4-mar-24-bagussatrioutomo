...
function calculate() {
	...
	let gender = document.getElementById("rb_gender").value;
	if (document.getElementById("rb_gender").checked) {
		if (isNaN(weight) || isNaN(age) || isNaN(height)) {
			alert("Column must be filled out");
		} else {
			// menghitung nilai BMI
			height_in_meter = height / 100;
			bmi_value = weight / (height_in_meter * height_in_meter);
